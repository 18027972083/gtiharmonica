...
<h3>曲谱从哪来</h3>
<ul>
<li><b>导入现成曲谱</b>：右上角「导入 MIDI / 曲谱」，或直接把 .mid / .midi / .json
    拖进窗口。</li>
<li><b>导入键位简谱</b>：右上角「导入简谱」，粘文本 → 预览试听 → 加入曲库。</li>
<li><b>自己改</b>：选中曲子点「编辑乐谱」，改完点「保存曲谱」写回曲库。</li>
<li><b>从音频 / 歌声转</b>：见下面一节。</li>
</ul>
<p>MIDI 入库时会让你二选一：<b>直转</b>（保留声部，编曲简单、旋律清晰的曲子用）或
<b>旋律化</b>（先按十六分网格量化、每个时刻只留最高的音，编曲密集、声部多的曲子用）。
两种都可以各来一份存着，对比着听。</p>

<h3>没有 MIDI 怎么办：把歌声 / 音频转成 MIDI</h3>
<p>本程序只认 MIDI / JSON 曲谱，不直接读音频。网上找不到 MIDI 时，可以用开源工具
<b>GAME</b>（OpenVPI 团队做的歌声转 MIDI 模型，免费）把歌里的主旋律扒成 MIDI，
再拖进本程序。</p>
...
<li>从整首歌里扒主旋律，通常<b>先把人声分离出来</b>（UVR5 之类的工具）再转，
    混着伴奏效果会明显打折。</li>
<li>实测：4.5 秒单声道音频，模型加载后约 1.5 秒转完（CPU）。Execution Provider
    切成 DirectML 可以用显卡加速。</li>
</ul>

<p><b>3. 导进本程序</b>：把生成的 .mid 拖进窗口，选「旋律化」或「直转」即可。
扒出来的音常见 ±1 半音漂移，在编辑器里点「调内吸附」就能修。</p>
