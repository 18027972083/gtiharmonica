...
ANNOUNCE_ID = '2026-09-22-v116-fix'

ANNOUNCEMENT_HTML = """
<h2>更新公告 · v1.1.6</h2>
<p>这一版修两个会毁曲谱的问题：</p>
<ul>
<li><b>「覆盖原曲」不会再把 MIDI 写坏</b>：原曲是 .mid 时，保存改为
    「另存为新曲谱」，MIDI 原文件保持原样。以前这条路径会把曲谱 JSON
    写进 .mid，那首曲子之后打开只会报「无法读取曲谱」，原文件也一起没了。</li>
<li><b>「剪掉时间」动手前先确认</b>：剪口里的音符会一起删掉，<b>跨在剪口上的
    长音也是</b>（长音整个消失、后面整段前移）。现在会先弹确认，写清要删
    几个音符、最长的那个有几秒 —— 想只删空白就取消，把两个点都点在
    完全没有音符的地方。</li>
<li><b>已经中招的曲谱能救回来</b>：后缀是 .mid、内容其实是曲谱 JSON 的
    文件，现在按内容读取（不再只看后缀），照样能打开、能试听。</li>
</ul>
