...
<p>MIDI 入库时会让你三选一（推荐「单轨提取」，网上找的原琴谱 / 改编 MIDI 用这个）：<b>直转</b>（保留声部，编曲简单、旋律清晰的曲子用）或
<b>旋律化</b>（先按十六分网格量化、每个时刻只留最高的音，编曲密集、声部多的曲子用）。
两种都可以各来一份存着，对比着听。</p>

<h3>音频转曲谱（内置，不用装外部工具）</h3>
    ...
    def __init__(self, files: List[str], parent=None):
        ...
        options = [
            ('direct', '直转 · 保留声部',
             ...
             '适合这一种，结果通常最接近原曲。'),
            ('track', '单轨提取 · 推荐（原琴谱 / 网上的 MIDI）',
             '从多轨里挑出主旋律那一条，压成单音，并把时值对齐到 120BPM 的'
             '十六分网格。网上找的原琴谱 / 钢琴改编 MIDI 用这个 —— 实测与'
             '人工整理好的口琴谱旋律一致度 0.11 半音/步，节奏也整齐。'),
            ('melody', '旋律化 · 先简化再入库',
